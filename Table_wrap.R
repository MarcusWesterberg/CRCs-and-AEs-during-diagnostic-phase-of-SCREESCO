...
tablepart <- function(X,
  ...
  if(continuous){
    ...
    if(add_info){
      ai  <- c(paste0(round2(mean(y,na.rm = TRUE),dig=dig)," (",round2(sd(y,na.rm = TRUE),dig=dig),")"),"")
      q   <- round2(quantile(y,p=c(0.5,0.25,0.75),na.rm=TRUE),dig=dig )
    ...
    if(cut_it){
      ...
      yct  <- table(yc)
      ycp  <- paste0("(",round2(yct/n*100,dig=dig_percent),")")
      if(!percent){ycp<-""}
    ...
    yp  <- paste0("(",round2(yt/n*100,dig=dig_percent),")")
    if(!percent){yp<-""}
  ...
  if(missing){
    mis <- sum(is.na(y))
    temp <- cbind("Missing",mis,paste0("(",round2(100*mis/n,dig=dig_percent),")"))
    colnames(temp)<-NULL
    ret[[length(ret)+1]] <- temp
  }
