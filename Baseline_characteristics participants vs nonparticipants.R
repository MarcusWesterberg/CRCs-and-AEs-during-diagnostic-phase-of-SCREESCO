###
# Baseline characteristics etc in participants vs nonparticipants
...
df_fit_raw <- haven::read_sas("S:\\SCREESCO\\Data\\SoS\\Screesco\\sos_df_fit.sas7bdat")
df_fit  <- df_fit_raw %>%
  filter(fit_round1_returned %in% "TRUE" | fit_round2_returned %in% "TRUE")
length(unique(df_fit$LopNr))
...
D$participant <- FALSE
D$participant[D$rand_arm %in% "dk" & !is.na(D$first_screesco_col)] <- TRUE

D$participant[D$rand_arm %in% "fit" & D$LopNr %in% df_fit$LopNr] <- TRUE

C = cbind(D$rand_arm %in% "dk" & D$participant,
          D$rand_arm %in% "dk" & !D$participant,
          D$rand_arm %in% "fit" & D$participant,
          D$rand_arm %in% "fit" & !D$participant)

DD <- D 

cn <- c("PCOL arm participant",
        "PCOL arm nonparticipant",
        "FIT arm participant",
        "FIT arm nonparticipant")
...
# Diagnosed CRC
table_1$crc <- table_wrap(X=DD,
                         varname="CRC_cens",
                         variable_name="Any colorectal cancer",
                         C=C,
                         cn=cn,
                         continuous=FALSE,
                         labels=c("No","Yes"),
...
table_1$crc1 <- table_wrap(X=DD,
                            varname="CRC_cens_stage12",
                            variable_name="Stage I-II",
                            C=C,
                            cn=cn,
                            continuous=FALSE,
                            labels=c("No","Yes"),
...
table_1$crc3 <- table_wrap(X=DD,
                           varname="CRC_cens_stage34",
                           variable_name="Stage II-IV",
                           C=C,
                           cn=cn,
                           continuous=FALSE,
                           labels=c("No","Yes"),
...
table_1$cvd <- table_wrap(X=DD %>% mutate(AE_cens_Cardiovascular=AE_cens_Cardiovascular %in% 1),
                         varname="AE_cens_Cardiovascular",
                         variable_name="Cardiovascular event",
                         C=C,
                         cn=cn,
                         continuous=FALSE,
                         labels=c("No","Yes"),
...
table_1$ge <- table_wrap(X=DD %>% mutate(AE_cens_Gastrointestinal=AE_cens_Gastrointestinal %in% 1),
                            varname="AE_cens_Gastrointestinal",
                            variable_name="Gastrointestinal event",
                            C=C,
                            cn=cn,
                            continuous=FALSE,
                            labels=c("No","Yes"),
...
table_1$death <- table_wrap(X=DD  %>% mutate(Censor=Censor %in% 1),
                            varname="Censor",
                            variable_name="Death from any cause",
                            C=C,
                            cn=cn,
                            continuous=FALSE,
                            labels=c("No","Yes"),
...
openxlsx::write.xlsx(table_1,file=".\\Resultat\\Table_participants.xlsx")
